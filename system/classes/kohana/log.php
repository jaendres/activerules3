...
class Kohana_Log {
	...
	{
		$log_write_levels = Site::conf('log.write_levels');

        if(in_array($type, $log_write_levels))
        {
            // Do any value replacement in message
            if ($values)
            {
            ...
            if (Site::conf('log.fire_php', FALSE) == TRUE
                    AND Site::conf('fire_php.enabled', FALSE) == TRUE)
            {
                switch($type)
                {
                    case 'error':
            ...
            if (Site::conf('log.kohana', FALSE) == TRUE)
            {
